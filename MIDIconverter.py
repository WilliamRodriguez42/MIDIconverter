...
def square(x):
    return np.clip(np.sin(x), -0.001, 0.001) * 100
...
class MConv:
    ...
    def __init__(self, volume=0.5, sample_rate=44100):
        ...
        self.samples = np.zeros(100) # Instantiate the samples with something so that we don't get an error if we try to write without reading
